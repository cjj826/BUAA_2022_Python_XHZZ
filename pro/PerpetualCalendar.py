	...
	for j in range(6):
		for k in range(7):
			...
			if j == 0 and k < blank:  # 上月
				calendar.labs[j][k].getLabel().setText(font(MAX_DAYSinMONTH_TABLE[i - 1] - blank + k + 1, LABEL_SIZE) + font(rq))
			else:  # 本月
				if day <= MAX_DAYSinMONTH_TABLE[i]:
					if year == 1582 and i == 9 and day > 4:
						if day < 15: continue
						else: calendar.labs[j + (k - 10) // 7][k - 3].getLabel().setText(font(day, LABEL_SIZE, "black", 800) + font(rq))
					else:
						calendar.labs[j][k].getLabel().setText(font(day, LABEL_SIZE, "black", 800) + font(rq))  # "500;font-family:微软雅黑"
				else:  # 次月
					if year == 1582 and i == 9:
						calendar.labs[j + (k - 10) // 7][k - 3].getLabel().setText(font(day - MAX_DAYSinMONTH_TABLE[i], LABEL_SIZE) + font(rq))
						if day - MAX_DAYSinMONTH_TABLE[i] == 11:
							for m in range(10): calendar.labs[j - 1 + (m + k - 2) // 7][(m + k - 2) % 7].getLabel().setText("")
					else: calendar.labs[j][k].getLabel().setText(font(day - MAX_DAYSinMONTH_TABLE[i], LABEL_SIZE) + font(rq))
	# 显示节日
	qmDay = 0
	for jq in jqb[i]:  # 节气
		jqrx = jq[0] + blank - 1
		if year == 1582 and i == 9: jqrx -= 10
		calendar.labs[jqrx // 7][jqrx % 7].getLabel().setText(font(jq[0], LABEL_SIZE, "black", 800) + font(SOLAR_TERMS_TABLE[jq[1]], 8, "red"))
		if jq[1] == 7: qmDay = jq[0]
	if year >= 1949:  # 公历节日起始年
		if qmDay: calendar.labs[(qmDay + blank - 1) // 7][(qmDay + blank - 1) % 7].getLabel().setText(font(qmDay, LABEL_SIZE, "red", 800) + font("清明", 8, "red"))
		for fes in GREGORIAN_FESTIVALS:
			if fes[0] == month + 1:
				jqrx = fes[1] + blank - 1
				calendar.labs[jqrx // 7][jqrx % 7].getLabel().setText(font(fes[1], LABEL_SIZE, "red", 800) + font(fes[2], 8, "red"))
	if year > 1911:  # 农历节日起始年
		...
		for fes in LUNAR_FRSTIVALS:
			...
			if fes[0] == ymb[yx1] and jqrx >= rqx1:  # 该月农历首日
				jqrx += -rqx1 + blank
				calendar.labs[jqrx // 7][jqrx % 7].getLabel().setText(font(jqrx - blank + 1, LABEL_SIZE, "red", 800) + font(fes[2], 8, "red"))
				...
				if 0 < jqrx <= MAX_DAYSinMONTH_TABLE[i] + blank: calendar.labs[jqrx // 7][jqrx % 7].getLabel().setText(font(jqrx - blank + 1, LABEL_SIZE, "red", 800) + font(fes[2], 8, "red"))
...
def displayDate(calendar, init = 0):
	...
	calendar.labInfo.setText("<br/>JDN {}<br/>{}<br/>{}<br/>{}月 星期{}<br/>{}{}{}年 〖{}〗<br/>{}月 {}日<br/><br/>".format(
		jdn, font(difference, 15, "black"), nm, month+1, week, font(day, 15, "black"), font(ym+rq, 15, "black"), ngz, sxm, ygz, rgz))
